...
class Config(object):
    ...
    def __init__(self, source_organ, target_organ, bucket_step, run_no):
        ...
        self.log_dir = "/l/users/shikhar.srivastava/workspace/hover_net/logs/%s/test/second_order/%s/ckpts/%s-%s/" % (self.run_no, self.bucket_step, self.source_organ, self.target_organ) # where checkpoints will be saved
        ...
        module = importlib.import_module(
            "models.%s.opt_test" % model_name
        )

        pretrained_path = f'/l/users/shikhar.srivastava/workspace/hover_net/logs/{self.run_no}/second_order/{self.bucket_step}/ckpts/{self.source_organ}-{self.target_organ}/net_epoch=50.tar'
